...
if not using_21_joint:
    motion_files = glob.glob(
        os.getenv("HOME") + "/workspace/lumos_ws/humanoid_demo_retarget/data/lus2_joint27/pkl/Append_Select_S29_*"
    )
    motion_files = glob.glob(
        os.getenv("HOME")
        + "/workspace/lumos_ws/humanoid_demo_retarget/sources/data/motions/lus2_joint27/pkl/CMU_CMU_12_12_04_poses*"
    )
else:
    # 21 joints
    ...
    motion_files = glob.glob(
        os.getenv("HOME")
        + "/workspace/lumos_ws/humanoid_demo_retarget/sources/data/motions/lus2_joint21/pkl/fallAndGetUp1_subject1.pkl"
    )
...
INIT_STATE_FIELDS = (
    INIT_ROOT_STATE_FIELDS
    + [key + "_dof_pos" for key in all_joint_names]
    + [key + "_dof_vel" for key in all_joint_names]
)
...
upper_joint_name = (
    ...
    + []
    if using_21_joint
    else ["left_wrist_yaw_joint", "right_wrist_yaw_joint", "left_wrist_roll_joint", "right_wrist_roll_joint"]
)
...
expressive_goal_fields += (
    [key1 + key2 for key1 in expressive_link_name for key2 in ["_pos_x_b", "_pos_y_b", "_pos_z_b"]]
    if expressive_link_name is not None
    else []
)
expressive_goal_fields += (
    [key1 + key2 for key1 in expressive_link_name for key2 in ["_vel_x_b", "_vel_y_b", "_vel_z_b"]]
    if expressive_link_name is not None
    else []
)
...
ref_motion_cfg = RefMotionCfg(
    ...
    time_between_frames=0.008,  # time between two frames of state and next_state
